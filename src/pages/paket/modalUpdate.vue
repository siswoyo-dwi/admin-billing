...
export default {
  ...
  methods: {
    ...
    async submit(){
      ...
      const res = await vm.$axios.post('paket/update', vm.dataForm)
      if(res.data.status == 200){
        vm.visible = false
        vm.$emit('refresh')
        vm.$toast.add({ severity: 'success', summary: 'Konfirmasi', detail: 'Berhasil update paket ps', life: 3000 });
      }else{
        vm.$toast.add({ severity: 'error', summary: 'Konfirmasi', detail: 'Gagal update paket ps', life: 3000 });
      }
    },
  },
};
